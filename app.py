...
def obter_base_dados_esgotados():
    ...
    try:
        ...
        colunas = ['Número de registo', 'Nome do medicamento', 'Data de início de rutura', 'Data prevista para reposição', 'TimeDelta', 'Data da Consulta']
        df_esgotados = df_esgotados[colunas].copy()

        df_esgotados['Número de registo'] = df_esgotados['Número de registo'].astype(str)
        df_esgotados['TimeDelta'] = pd.to_numeric(df_esgotados['TimeDelta'], errors='coerce')
        ...
        df_esgotados['Data prevista para reposição'] = pd.to_datetime(df_esgotados['Data prevista para reposição'])
...
def load_produtos_nao_comprar(_dict_locs):
    ...
    try:
        ...
        nc_df['FARMACIA'] = nc_df['FARMACIA'].apply(lambda x: mapear_localizacao(x, _dict_locs))
...
def criar_tabela_dimensao(df):
    ...
    df_univ_bruta['DESIGNAÇÃO'] = df_univ_bruta['DESIGNAÇÃO'].apply(limpar_designacao)
    df_univ = df_univ_bruta.drop_duplicates(subset=['CÓDIGO'], keep='first').reset_index(drop=True)
...
def processar_ficheiros_upload(ficheiros, labs_selecionados, codigos_txt, _dicionario_labs, _dict_locs):
    ...
    if lista_df:
        ...
        df_final['LOCALIZACAO'] = df_final['LOCALIZACAO'].apply(lambda x: mapear_localizacao(x, _dict_locs))
        ...
        df_final['CÓDIGO_NUM'] = pd.to_numeric(df_final['CÓDIGO'], errors='coerce')
        
        # Identificar as linhas inválidas
        mask_invalid = df_final['CÓDIGO_NUM'].isna()
        if mask_invalid.any():
            codigos_invalidos = df_final.loc[mask_invalid, 'CÓDIGO'].unique().tolist()
...
def sellout_total(dataframe_combinada, df_master_produtos):
    ...
    colunas_nao_somar = ['CÓDIGO', 'DESIGNAÇÃO', 'LOCALIZACAO', 'PVP', 'P.CUSTO', 'DUC', 'DTVAL', 'CLA']
    colunas_agregar = [col for col in dataframe_combinada.columns if col not in colunas_nao_somar]

    # 1. Agregar Vendas (Soma) por CÓDIGO
    grouped_df = dataframe_combinada.groupby('CÓDIGO')[colunas_agregar].sum().reset_index()
    ...
    grouped_df = pd.merge(grouped_df, df_master_produtos, on='CÓDIGO', how='left')
    grouped_df['DESIGNAÇÃO'] = grouped_df['DESIGNAÇÃO'].str.title()

    # 6. Renomear colunas de preço para a vista agrupada
    grouped_df = grouped_df.rename(columns={'PVP': 'PVP_Médio', 'P.CUSTO': 'P.CUSTO_Médio'})
    ...
    grouped_df = grouped_df.sort_values(by=['DESIGNAÇÃO', 'CÓDIGO'], ascending=[True, True])
...
def combina_e_agrega_df(dataframe_combinada, df_master_produtos):
    ...
    dataframe_combinada = pd.concat([dataframe_combinada, grouped_df], ignore_index=True)

    # 6. Reintroduzir a DESIGNAÇÃO limpa
    dataframe_combinada = pd.merge(dataframe_combinada, df_master_produtos, on='CÓDIGO', how='left')
...
def unir_sell_out_com_esgotados(df_sell_out, df_esgotados):
    df_completa = df_sell_out.merge(
        df_esgotados[['Número de registo', 'Data de início de rutura', 'Data prevista para reposição', 'TimeDelta']], 
        left_on='CÓDIGO', right_on='Número de registo', how='left'
    )
    df_completa.drop(columns='Número de registo', axis=1, inplace=True, errors='ignore')
    return df_completa
...
def calcular_proposta_esgotados(df, col_media='Media', col_timedelta='TimeDelta', col_stock='STOCK', col_proposta='Proposta'):
    ...
    df.loc[mask, col_proposta] = ((df.loc[mask, col_media] / 30) * df.loc[mask, col_timedelta] - df.loc[mask, col_stock]).round(0).astype(int)
    return df

def unir_df_na_comprar_a_df_clean(df_clean, df_nao_comprar):
    df_clean['CÓDIGO_STR'] = df_clean['CÓDIGO'].astype(str)
    df_clean = df_clean.merge(df_nao_comprar, left_on=['CÓDIGO_STR', 'LOCALIZACAO'], right_on=['CNP', 'FARMACIA'], how='left')
    df_clean.rename(columns={'DATA': 'DATA_OBS'}, inplace=True)
    df_clean.drop(columns=['CNP', 'FARMACIA', 'CÓDIGO_STR'], axis=1, inplace=True, errors='ignore')
    return df_clean
...
def processar_logica_negocio(df, df_esgotados, df_nao_comprar, cols_selecionadas, pesos, valor_previsao, agrupado=False):
    ...
    if not df_esgotados.empty:
        try:
            ...
            df_esgotados_copy['Número de registo'] = df_esgotados_copy['Número de registo'].astype(str)
            
            df = df.merge(
                df_esgotados_copy[['Número de registo', 'Data de início de rutura', 'Data prevista para reposição', 'TimeDelta']], 
                left_on='CÓDIGO_STR', right_on='Número de registo', how='left'
            )
            df.drop(columns=['Número de registo', 'CÓDIGO_STR'], axis=1, inplace=True, errors='ignore')
            
            df = calcular_proposta_esgotados(df)

            if 'Data de início de rutura' in df.columns:
                df['Data de início de rutura'] = pd.to_datetime(df['Data de início de rutura']).dt.strftime('%d-%m-%Y')
            if 'Data prevista para reposição' in df.columns:
                df['Data prevista para reposição'] = pd.to_datetime(df['Data prevista para reposição']).dt.strftime('%d-%m-%Y')

            df.rename(columns={'Data de início de rutura': 'DIR', 'Data prevista para reposição': 'DPR'}, inplace=True)
            if 'TimeDelta' in df.columns:
                df.drop(columns='TimeDelta', axis=1, inplace=True)
        except Exception as e:
            st.warning(f"⚠️ Ocorreu um erro ao unir a dataframe de esgotados. {str(e)}")
    ...
    if not df_nao_comprar.empty:
        try:
            df['CÓDIGO_STR'] = df['CÓDIGO'].astype(str)
            if agrupado:
                df_nc_unique = df_nao_comprar[['CNP', 'DATA']].sort_values('DATA', ascending=False).drop_duplicates(subset=['CNP'], keep='first')
                df = df.merge(df_nc_unique, left_on='CÓDIGO_STR', right_on='CNP', how='left')
                df.rename(columns={'DATA': 'DATA_OBS'}, inplace=True)
                if 'CNP' in df.columns: df.drop(columns=['CNP'], axis=1, inplace=True)
            else:
                df = df.merge(df_nao_comprar, left_on=['CÓDIGO_STR', 'LOCALIZACAO'], right_on=['CNP', 'FARMACIA'], how='left')
                df.rename(columns={'DATA': 'DATA_OBS'}, inplace=True)
                df.drop(columns=['CNP', 'FARMACIA'], axis=1, inplace=True, errors='ignore')
            df.drop(columns=['CÓDIGO_STR'], axis=1, inplace=True, errors='ignore')
        except Exception as e:
...
def formatar_excel(dataframe_final):
    ...
    fill_roxo = PatternFill(start_color="E6D5F5", end_color="E6D5F5", fill_type="solid")
    ...
    col_localizacao = headers.index('LOCALIZACAO') + 1 if 'LOCALIZACAO' in headers else None
    col_data_obs = headers.index('DATA_OBS') + 1 if 'DATA_OBS' in headers else None
    col_dir = headers.index('DIR') + 1 if 'DIR' in headers else None
    col_proposta = headers.index('Proposta') + 1 if 'Proposta' in headers else None
...
def render_sidebar():
    ...
    st.sidebar.markdown("<small>Selecione um ou mais laboratórios para filtrar (Ignorado se usar txt abaixo).</small>", unsafe_allow_html=True)
    ...
    st.sidebar.markdown("<small>Carregue um ficheiro .txt com um código por linha. Se usar isto, o filtro por laboratório será ignorado.</small>", unsafe_allow_html=True)
    ficheiro_codigos = st.sidebar.file_uploader("Ficheiro de Códigos (.txt)", type=['txt'], key="upload_txt_codigos")
    ...
    processar = st.sidebar.button("🚀 Processar Dados", width='stretch', type="primary")
...
def main():
    ...
    with st.expander("🔬 Ver Códigos (CLA) dos Laboratórios Selecionados"):
        if opcoes_sidebar['labs_selecionados']:
            st.write("Os seguintes códigos estão a ser utilizados para filtrar as vendas:")
    ...
    if opcoes_sidebar['processar']:
        if opcoes_sidebar['ficheiros_infoprex']:
            with st.spinner("A processar e agrupar ficheiros base..."):
                ...
                if df_base.empty:
                    st.warning("Não há dados válidos para processar após aplicar os filtros.")
                    st.session_state.df_base_agrupada = pd.DataFrame()
                    st.session_state.df_base_detalhada = pd.DataFrame()
                else:
                    ...
                    st.session_state.last_txt_name = opcoes_sidebar['ficheiro_codigos'].name if opcoes_sidebar['ficheiro_codigos'] else None
        else:
